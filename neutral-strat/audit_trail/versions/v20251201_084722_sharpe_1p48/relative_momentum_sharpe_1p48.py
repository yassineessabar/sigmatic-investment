...
def backtest_relative_momentum_pair(
    ...
    if enable_optimizations:
        # Advanced drawdown protection for low DD target
        ...
        # Multi-tier aggressive drawdown protection for <19% target
        # Tier 1: Early warning at 5% DD
        early_threshold = -0.05
        early_multiplier = np.where(drawdown < early_threshold, 0.85, 1.0)

        # Tier 2: Moderate protection at 10% DD
        moderate_threshold = -0.10
        moderate_multiplier = np.where(drawdown < moderate_threshold, 0.65, 1.0)

        # Tier 3: Aggressive protection at 15% DD
        aggressive_threshold = -0.15
        aggressive_multiplier = np.where(drawdown < aggressive_threshold, 0.40, 1.0)

        # Tier 4: Emergency protection at 18% DD
        emergency_threshold = -0.18
        emergency_multiplier = np.where(drawdown < emergency_threshold, 0.20, 1.0)

        # Combine all protection tiers
        total_dd_protection = (early_multiplier * moderate_multiplier *
                              aggressive_multiplier * emergency_multiplier)

        # Additional volatility-based protection
        short_vol = strategy_returns_preview.rolling(10).std()
        vol_median = short_vol.rolling(60).median()
        vol_multiplier = np.where(
            short_vol > vol_median * 2, 0.7,  # High vol = reduce positions
            np.where(short_vol > vol_median * 1.5, 0.85, 1.0)
        )

        # Loss streak protection
        loss_streak = (strategy_returns_preview < 0).rolling(3).sum()
        streak_multiplier = np.where(
            loss_streak >= 3, 0.6,  # 3+ losses in a row
            np.where(loss_streak >= 2, 0.8, 1.0)  # 2+ losses
        )

        # Apply all protections
        final_multiplier = (total_dd_protection *
                           vol_multiplier.fillna(1.0) *
                           streak_multiplier.fillna(1.0))

        aligned_data['btc_weight'] *= final_multiplier
        aligned_data['alt_weight'] *= final_multiplier
