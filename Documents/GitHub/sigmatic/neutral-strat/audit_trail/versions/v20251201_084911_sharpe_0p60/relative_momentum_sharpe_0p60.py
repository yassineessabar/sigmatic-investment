...
def backtest_relative_momentum_pair(
    ...
    # SIMPLE EFFECTIVE DRAWDOWN PROTECTION
    if enable_optimizations:
        # Calculate strategy returns for drawdown monitoring
        ...
        rolling_max = cumulative_ret.rolling(window=20, min_periods=1).max()
        drawdown = (cumulative_ret - rolling_max) / rolling_max

        # Simple but effective protection at 15% DD
        dd_threshold = -0.15  # 15% drawdown threshold
        dd_multiplier = np.where(drawdown < dd_threshold, 0.5, 1.0)  # Cut positions in half

        aligned_data['btc_weight'] *= dd_multiplier
        aligned_data['alt_weight'] *= dd_multiplier
