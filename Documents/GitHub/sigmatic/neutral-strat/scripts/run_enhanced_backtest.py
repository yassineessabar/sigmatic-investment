...
"""
Enhanced Relative Momentum Backtest Runner
Tests advanced technical analysis enhanced strategy
"""
...
# Import data fetching from original script
from scripts.run_relative_momentum_backtest import (
    fetch_ohlcv_all,
    fetch_funding_rates,
...
def run_enhanced_backtest(start_date=None, end_date=None,
                         config_path='config/unified_trading_config.yaml',
                         create_audit_version=True, description="",
                         optimization_notes="", enhancement_level="moderate"):
    """Run enhanced relative momentum backtest"""
    ...
    print(f"Enhanced Strategy Universe: {universe}")
    ...
    print(f"Enhanced Backtest ({enhancement_level}): {backtest_start_date} to {backtest_end_date}")

    # Convert to datetime
    start_dt = pd.to_datetime(backtest_start_date)
    end_dt = pd.to_datetime(backtest_end_date)

    # Fetch data efficiently (reuse cached data when possible)
    data = {}
    funding_data = {}
    ...
    for symbol in universe:
        ...
        print(f"📈 Loading enhanced data for {futures_symbol}...")
        ...
        try:
            ...
            if os.path.exists(price_fname):
                ...
                if len(existing_df) > 0:
                    ...
                    if len(df_filtered) > 100:  # Sufficient data
                        data[symbol] = df_filtered
                        print(f"→ Using cached data: {len(df_filtered)} candles")
            ...
            print(f"🔄 Fetching fresh data for {futures_symbol}...")
            df = fetch_ohlcv_all(
            ...
            if len(df_filtered) == 0:
                logger.warning(f"No data for {futures_symbol}")
                continue
            ...
            print(f"→ Fresh data: {len(df_filtered)} candles")
    ...
    print(f"\n🚀 Running ENHANCED Strategy (Level: {enhancement_level.upper()})...")
    ...
    for pair_config in config['pairs']:
        ...
        try:
            ...
            if config['strategy']['optimization']['enabled']:
                ...
                best_result = optimize_enhanced_strategy(
                    data[base_symbol], data[alt_symbol],
                    list(range(window_start, min(window_end, 25))),  # Limit range for speed
                ...
                best_result = backtest_enhanced_momentum(
                    data[base_symbol], data[alt_symbol],
            ...
            # Print results
            stats = print_enhanced_stats(pair_name, best_result, freq)
            summary_rows.append(stats)
            pair_returns[pair_name] = best_result['returns']

        except Exception as e:
            logger.error(f"Failed to backtest enhanced {pair_name}: {e}")
            continue

    if not pair_returns:
        logger.error("No successful enhanced backtests completed")
        return
    ...
    def calculate_portfolio_metrics(returns, name):
        ...
        return {
            'Pair': name,
            'EMA_Window': f'Enhanced-{enhancement_level}',
            'Ann.Return': ann_ret,
            'Vol': ann_vol,
            'Sharpe': sharpe,
            'Calmar': calmar,
    ...
    eq_stats = calculate_portfolio_metrics(equal_portfolio_returns, f"Equal-Weight Enhanced-{enhancement_level}")
    vs_stats = calculate_portfolio_metrics(vol_scaled_portfolio_returns, f"Vol-Scaled Enhanced-{enhancement_level}")
    ...
    results_table.to_csv(f"results/enhanced_{enhancement_level}_results.csv", index=False)

    print(f"\n======= ENHANCED-{enhancement_level.upper()} STRATEGY PERFORMANCE =======")
    print(results_table.to_string(index=False))
    print(f"\n✅ Results saved to results/enhanced_{enhancement_level}_results.csv")
    ...
    if audit_trail and create_audit_version:
        try:
            ...
            version_id = audit_trail.create_strategy_version(
                ...
                description=description or f"Enhanced-{enhancement_level} {backtest_start_date} to {backtest_end_date}",
                optimization_notes=optimization_notes or f"Advanced technical analysis with {enhancement_level} risk profile"
            )

            print(f"\n📋 Created Enhanced audit trail version: {version_id}")

        except Exception as e:
            logger.warning(f"Failed to create enhanced audit trail: {e}")
    ...
    if len(benchmark_returns) > 0:
        ...
        print_performance_comparison(
            ...
            f"Enhanced-{enhancement_level} Equal-Weight", "BTC Buy & Hold", freq,
            initial_capital, None
        )

    # Plot results
    print(f"\nEnhanced Backtest Range: {df_all.index.min().strftime('%Y-%m-%d')} → {df_all.index.max().strftime('%Y-%m-%d')}")
...
def print_enhanced_stats(pair, result, freq=365):
    """Print enhanced strategy statistics"""

    ann_return = result.get('ann_return', 0)
    ann_vol = result.get('ann_vol', 0)
    sharpe = result.get('sharpe', 0)
    max_dd = result.get('max_dd', 0)
    enhancement_level = result.get('enhancement_level', 'moderate')
    ...
    print(f"\n========== {pair} (Enhanced-{enhancement_level} Strategy) ==========")
    print(f"Best EMA window: {result['ema_window']}d | Enhancement: {enhancement_level}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Run enhanced relative momentum strategy backtest')
    ...
    parser.add_argument('--enhancement-level', type=str, choices=['conservative', 'moderate', 'aggressive'],
                       help='Enhancement level', default='moderate')
